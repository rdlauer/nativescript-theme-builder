$(function () {
	...
	$('#global-font').on('change', function () {
		...
		if (os == 'ios') {
			$('.device-screen')
				.contents()
				.find('body, h1, h2, h3, h4, h5, h6, button, .tab-item, div')
				.css('style', 'font-family: San Francisco');
		} else if (os == 'android') {
			$('.device-screen')
				.contents()
				.find('body, h1, h2, h3, h4, h5, h6, button, .tab-item, div')
				.css('style', 'font-family: Roboto');
		}

		// header
		if (os == 'ios') {
			$(".iphonex-time, .iphonex-icons").show();
			$('.device-screen').contents().find('.bar-positive').removeClass('header-md');
			$('.device-screen').contents().find('.bar .title').removeClass('no-notch');
			$('.device-screen').contents().find('.scroll-content').removeClass('scroll-content-android');
		} else if (os == 'android') {
			$(".iphonex-time, .iphonex-icons").hide();
			$('.device-screen').contents().find('.bar-positive').addClass('header-md');
			$('.device-screen').contents().find('.bar .title').addClass('no-notch');
			$('.device-screen').contents().find('.scroll-content').addClass('scroll-content-android');
		}
		...
		if (os == 'ios') {
			...
			$('#ui-widgets')
				.contents()
				.find('#android-activity')
				.hide();
			// slider
			$('#ui-widgets')
				.contents()
				.find('#slider')
				.removeClass('range-md')
				.addClass('range-ios');
		} else if (os == 'android') {
			// segmented bar
			$('#ui-widgets')
		...
		if (os == 'ios') {
			$('#ui-buttons')
				.contents()
				.find('.button')
				.removeClass('btn-android-shadow btn-android-text');
		} else if (os == 'android') {
			$('#ui-buttons')
				.contents()
				.find('.btn-primary, .button-outline')
				.addClass('btn-android-shadow');
			$('#ui-buttons')
				.contents()
				.find('.button')
				.addClass('btn-android-text');
		}
		...
		if (os == 'ios') {
			$('#ui-listviews')
				.contents()
				.find('.tabs-positive')
				.removeClass('tabs-striped tabs-top');
			$('#ui-listviews')
				.contents()
				.find('.list-tabs-content')
				.removeClass('list-tabs-content-android');
			$('#ui-tabs')
				.contents()
				.find('.tabs-positive')
				.removeClass('tabs-striped tabs-top');
			$('#ui-tabs')
				.contents()
				.find('.tabs-content')
				.removeClass('tabs-content-android');
			$('#ui-tabs')
				.contents()
				.find('.no-notch-items')
				.removeClass('no-notch-items-android');
			$('#ui-tabs')
				.contents()
				.find('.bar .button')
				.removeClass('tabs-bar-button-android');
		} else if (os == 'android') {
			$('#ui-listviews')
				.contents()
				.find('.tabs-positive')
				.addClass('tabs-striped tabs-top');
			$('#ui-listviews')
				.contents()
				.find('.list-tabs-content')
				.addClass('list-tabs-content-android');
			$('#ui-tabs')
				.contents()
				.find('.tabs-positive')
				.addClass('tabs-striped tabs-top');
			$('#ui-tabs')
				.contents()
				.find('.tabs-content')
				.addClass('tabs-content-android');
			$('#ui-tabs')
				.contents()
				.find('.bar .button')
				.addClass('tabs-bar-button-android');
			$('#ui-tabs')
				.contents()
				.find('.no-notch-items')
				.addClass('no-notch-items-android');
		}
	});
	...
	document
		.getElementById('file')
		.addEventListener('change', readCSSFile, false);
});
		...
		$.each(data, function (index, value) {
			if (id == value.id) {
					...
					$.each(value.web, function (index, valueWeb) {
						...
						if (valueWeb.class == '.range input') {
							$('#' + iframe)
								...
								.css(
									valueWeb.attr,
									'linear-gradient(to right,' + str + ' 0,' + str + ' 100%)'
								);
						} else {
							$('#' + iframe)
								.contents()
								.find(valueWeb.class)
								.css(valueWeb.attr, str);
						}
					});
				});
				...
				if (id == 'global-ab') {
					...
					$('.iphonex-time, .iphonex-icons').css('color', idealTextColor(str));
					$('.device-screen')
						.contents()
						.find('.pane .bar.bar-positive .title, .ion-navicon')
						.css('color', idealTextColor(str));
				} else if (id == 'ios-seg-bar') {
					// when setting segmented bar text, add contrasting ios "selected" text color
					$('#ui-widgets')
						.contents()
						.find('.segment-ios .segment-button.segment-activated')
						.css('color', idealTextColor(str));
				}
