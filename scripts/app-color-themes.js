$(function () {
	...
	// enable the nav left/right to cycle through color palettes
	$("#btn-color-left").click(function () {
		var val = parseInt($('input[name=color-scheme]:checked').val()) - 1;
		$("input[name=color-scheme][value=" + val + "]").iCheck('uncheck');
		if (val == 0) {
			val = 36;
		}
		$("input[name=color-scheme][value=" + val + "]").iCheck('check');
		$("input[name=color-scheme][value=" + val + "]").prop('checked', true);
		$('#theme-ok').click();
	});
	$("#btn-color-right").click(function () {
		var val = parseInt($('input[name=color-scheme]:checked').val()) + 1;
		$("input[name=color-scheme][value=" + val + "]").iCheck('uncheck');
		if (val == 37) {
			val = 1;
...
function acceptColorTheme() {
	...
	var widgetColor = $('#' + val + '-color-wd')
		.spectrum('get')
		.toHexString();
	var actionBarColor = $('#' + val + '-color-ab')
		.spectrum('get')
		.toHexString();
	var backgroundColor = $('#' + val + '-color-bg')
		.spectrum('get')
		.toHexString();
	...
	$.getJSON('scripts/data/relations.json', function (data) {
		$.each(data, function (index, value) {
			...
			if (cp === true) {
				if (cpId.match(/-ab$/) || cpId.indexOf('lbl') !== -1) {
					// actionbar color or a button label
					color = actionBarColor;
				} else if (cpId.match(/-bg$/)) {
					// background color
...
function isColor(cpId) {
	...
	if (
		...
		cpId.match(/-px$/) ||
		cpId == 'global-text' ||
		cpId == 'tab-label-text-color' ||
		cpId == 'buttons-primary-text-color' ||
		cpId == 'buttons-disabled-text-color' ||
		cpId == 'text-primary-color' ||
		cpId == 'text-danger-color' ||
		cpId == 'text-muted-color' ||
		cpId == 'drawer-header-text-color' ||
		cpId == 'drawer-item-text-color' ||
		cpId == 'drawer-item-sel-text-color' ||
		cpId == 'ios-seg-bar-text-ios-active'
	) {
		return false;
	} else {
		return true;
...
function readCSSFile(evt) {
	...
	reader.onload = function () {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			if (line.startsWith('.')) {
				// starts with "."? must be the class
				myClass = line;
			} else if (line.length > 0) {
				// otherwise, split the line on an empty space
				lineAV = line.split(' ');
				for (var ii = 0; ii < lineAV.length; ii++) {
					var l = lineAV[ii].trim();
					if (l.endsWith(':')) {
						// ends with ":"? must be attribute
						myAttr = l.replace(':', '');
					} else if (l.endsWith(';')) {
						// ends with ";"? must be value
						myVal = l.replace(';', '');
					}
				}
			}
